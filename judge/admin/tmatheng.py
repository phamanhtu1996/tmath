...
from judge.models import Exam, ExamProblem, ExamSubmission, Profile, Rating, Submission
...
class ExamForm(ModelForm):
    ...
    class Meta:
        widgets = {
            ...
            'private_Examants': AdminHeavySelect2MultipleWidget(data_view='profile_select2',
            ...
            'view_exam_scoreboard': AdminHeavySelect2MultipleWidget(data_view='profile_select2',
                                                                       attrs={'style': 'width: 100%'}),
            'description': AdminMartorWidget(attrs={'data-markdownfy-url': reverse_lazy('Exam_preview')}),
        }
...
class ExamAdmin(NoBatchDeleteMixin, VersionAdmin):
    fieldsets = (
        ...
        (_('Access'), {'fields': ('access_code', 'is_private', 'private_Examants', 'is_organization_private',
    ...
    def rate_all_view(self, request):
        ...
        with transaction.atomic():
            ...
            for contest in Exam.objects.filter(is_rated=True, end_time__lte=timezone.now()).order_by('end_time'):
    ...
    def rate_view(self, request, id):
        if not request.user.has_perm('judge.contest_rating'):
            raise PermissionDenied()
        contest = get_object_or_404(Exam, id=id)
